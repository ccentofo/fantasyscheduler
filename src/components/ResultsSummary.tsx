...
type StatTint = "default" | "danger" | "eagles" | "muted";
...
export default function ResultsSummary({ data, onReuseSeed }: ResultsSummaryProps) {
  if (!data) return null;
  const {
    seed,
    teams,
    ...
    <div className="space-y-5" data-testid="results-summary">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 pb-4 border-b border-[#222]">
        <div className="flex flex-wrap gap-5 sm:gap-6">
          <Stat label="Teams" value={teams ?? 0} testId="stat-teams" />
          <Stat label="Weeks" value={weeks ?? 0} testId="stat-weeks" />
          <Stat
            label="Rival Week"
      ...
      {rivalsEntries.length > 0 && (
        <div className="flex flex-wrap gap-2" data-testid="rivals-strip">
          {rivalsEntries.map(([a, b]: [string, string]) => (
            <div
              key={`${a}-${b}`}
...
function Stat({ label, value, tint = "default", testId }: StatProps) {
  const color =
    tint === "danger"
      ? "#ef4444"
      : tint === "eagles"
